...
```
speech-to-text/
├── transcribe.py            # entry point:  python transcribe.py [files] [options]
...
python transcribe.py call.wav folder/         # specific files or folders
python transcribe.py call.wav -f txt -f srt   # several output formats (txt, srt, json)
python transcribe.py call.wav --limit-seconds 60   # quick check on the first minute
python transcribe.py --help                   # all options
```

Each line is printed as soon as it is ready and the transcript is saved to
`transcripts/<recording name>.hinglish.txt`.

## Settings

Defaults live in `transcriber/config.py`; every one can be overridden on the command line.
...
| `--cpu-threads` | `0` | Library default. Try the number of physical cores if the CPU is under-used. |
...
`transcriber/hinglish/words.py` maps Devanagari words to the spelling you want, and is
